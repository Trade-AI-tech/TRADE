...
## ธีมสว่าง/มืด — สีอยู่ที่เดียว

สีทั้งแอปมาจากตัวแปรใน `src/styles/globals.css` ตัวเดียว โดย `:root` = ธีมสว่าง (ค่าเริ่มต้น)
และ `.dark` บน `<html>` = ธีมมืด (ค่าเดิมก่อนมีระบบสลับธีมทุกตัว) `tailwind.config.js`
ชี้คลาสของ Tailwind มาที่ตัวแปรพวกนี้ คลาสเดิมทั้งแอปจึงสลับตามธีมเองโดยไม่ต้องไล่แก้ทีละหน้า

**กติกาการเขียนสีในโค้ด** — ตัวแปรสีเก็บเป็น "ช่อง RGB" (`4 120 87`) ไม่ใช่ hex
เพื่อให้ Tailwind เติม alpha เองผ่าน `rgb(var(--x) / <alpha-value>)` ผลคือ:

| เขียนแบบนี้ | ผลลัพธ์ |
|---|---|
| `text-up` · `bg-surface-2` · `text-white` | ✅ ใช้โทเคน ดีที่สุด รองรับ `/opacity` ด้วย |
| `text-[rgb(var(--text-primary))]` | ✅ ใช้ได้ เมื่อไม่มีโทเคนให้ใช้ |
| `text-[var(--up)]` | ❌ ได้ `color: 4 120 87` เบราว์เซอร์ทิ้งทั้งบรรทัดเงียบ ๆ `tsc` ไม่จับ |
| `border-[var(--border-subtle)]` | ✅ ข้อยกเว้น — `--border-subtle` / `--invert` เก็บเป็นสีเต็ม ไม่ใช่ช่อง RGB |

ตรวจค่าจริงใน `globals.css` ก่อนตัดสินทุกครั้ง และวิธีเดียวที่พิสูจน์ได้ว่าสีมาจริงคือวัด
`getComputedStyle` บน production build — `tsc --noEmit` ผ่านสะอาดทั้งที่สีหายไปหมดก็ได้

**สีของ PWA (`public/manifest.webmanifest`)** — `theme_color` / `background_color`
ตั้งเป็น `#f7f8fa` ซึ่งคือ `--surface-0` ของ**ธีมสว่าง** เพราะ manifest เป็น JSON ล้วน
เปลี่ยนตามธีมไม่ได้ (และใส่คอมเมนต์อธิบายในไฟล์ก็ไม่ได้ จึงต้องเขียนไว้ที่นี่)
ต้องเลือกค่าเดียวจึงเลือกให้ตรงกับ**ธีมเริ่มต้น** ผู้ใช้ที่ยังไม่เคยกดสวิตช์จะได้หน้า splash
สีเดียวกับหน้าแอปจริง ส่วนคนที่เลือกธีมมืดไว้จะเห็น splash สว่างแวบหนึ่งตอนเปิดแอป
แล้วสคริปต์ใน `src/app/layout.tsx` จะสลับ `<meta name="theme-color">` เป็น `#0f1520`
ให้ทันทีที่หน้าโหลด (แถบสถานะและ UI ของเบราว์เซอร์จึงถูกต้อง มีแค่จอ splash ที่แก้ไม่ได้)
