 ...
 *
 * ═══ ⚠ อ่านก่อนสั่งรันไฟล์นี้ใหม่ (บันทึกเมื่อ 2026-08-29) ═════════════════════
 * ไฟล์ข้อมูลที่ ship อยู่ (src/lib/speed-scorecard.client.json) สร้างเมื่อ 2026-08-17
 * บนจักรวาล 57 ตัว = 114 คู่ (มีหุ้นสหรัฐ/คริปโต/หุ้นไทยอยู่ในนั้น) ซึ่งค้างเก่ามาตั้งแต่
 * ก่อนคำสั่ง "เทรดทองอย่างเดียว" (2026-08-29) แล้ว — ทับกับจักรวาลจริงแค่ 2 คู่
 *
 * ถ้ารันใหม่ตอนนี้: มันอ่าน SYMBOL_UNIVERSE ตัวจริง → เหลือ 2 คู่ (XAUUSD 1D + 1H)
 * และ usablePairs ของ 1H จะตกจาก 57 เหลือ 1 ซึ่งพลิก validationSummary/readerPolicy ได้
 * (และด่าน check:ui-claims มีเงื่อนไข "UI ใช้ตัวเลขรายคู่โดยไม่เช็ก readerPolicy" ที่ขยับตาม)
 *
 * คำแนะนำ: **ยังไม่ต้องรีบิลด์** ทองมีคู่ของตัวเองอยู่แล้วทั้ง 1D และ 1H ตัวอ่านจึงตอบ
 * ด้วยข้อมูลของทองเองอยู่แล้ว การรีบิลด์เปลี่ยนแค่ "ตัวเลขนโยบายรอบ ๆ" ไม่ได้ทำให้
 * คำตอบของทองแม่นขึ้น และมีโอกาสพลิกด่าน CI โดยไม่ได้ตั้งใจ
 * ถ้าจะรีบิลด์จริง ให้รัน npm run check:ui-claims + test:scorecard ทันทีหลังรัน
 */
 ...
 *   2. maxSignalsPerRun (โควตาต่อรอบ — ปัจจุบัน 3 อัน/รอบ) เป็นเกณฑ์ระดับ "รอบสแกน" ไม่ใช่ระดับไม้
