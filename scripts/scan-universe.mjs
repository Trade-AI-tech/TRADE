 ...
 * 2026-08-29 ทำให้เป็นค่าที่ตั้งใจ (คงไว้ที่ 20) · 2026-09-01 ลดเหลือ 4 ตามที่เจ้าของสั่ง
 *
 * ปริมาณแจ้งเตือนของทั้งระบบแขวนอยู่บนเลขนี้ตัวเดียว ไม่ใช่บนเกณฑ์คุณภาพ —
 * วัดจริงบนแท่งสด XAUUSD 15m ย้อนหลัง 31 วัน: ผ่านประตูคุณภาพ 402 ใบ = 12.97 ใบ/วัน
 * (ประตูปฏิเสธ 0 ใบ) แล้วหน้าต่างกันซ้ำเป็นตัวเดียวที่กดลงมา
 *
 * ⚠ อยากได้ถี่/ห่างกว่านี้ ให้หมุนที่ DEDUPE_HOURS_15M ข้างล่างเท่านั้น
 *   อย่าไปผ่อนเกณฑ์คุณภาพแทน — วัดแล้วว่าผ่อนเกณฑ์ทำให้แย่ลง ไม่ใช่ดีขึ้น
 *   (เลนนี้ยังไม่เคยถูกตรวจสอบนอกตัวอย่างเลย ดู perTimeframe ใน src/lib/universe.ts)
 *
 * ── 2026-09-01: เจ้าของสั่งลด 15m จาก 20 ชม. เหลือ 4 ชม. ────────────────────
 * ตารางที่วัดจากไม้จริง 410 ไม้ (scripts/research/reports/fast-lane-summary.md)
 * แล้วเสนอให้เลือก: 20 ชม. = 1.54 ใบ/วัน · 8 = 3.04 · 4 = 4.50 · 2 = 6.52 ·
 * 1 = 8.49 · 0 = 13.99 — เจ้าของเลือก 4 ชม.
 *
 * เลือก 4 เพราะมันเท่ากับ DEDUPE_HOURS_1H ที่มีอยู่แล้วในไฟล์นี้ จึงไม่มีเลขใหม่
 * ให้ต้องอธิบายทีหลัง และอยู่กึ่งกลางระหว่างค่าเดิมกับเพดานของเลน
 *
 * ⚠ สิ่งที่การเปลี่ยนนี้ **ไม่ใช่**: ไม่ใช่หลักฐานว่า 4 ชม. ทำเงินได้ดีกว่า 20 ชม.
 *   คอลัมน์ meanR ในตารางขึ้นลงแบบไม่มีทิศทาง ซึ่งเป็นสิ่งที่ควรคาดหวังเมื่อ MDE
 *   (0.27–0.34 R/ไม้) ใหญ่กว่าทุกความต่างที่วัดได้ · และเส้นฐาน "long ทุกไม้"
 *   ชนะเครื่องยนต์ใน 29/36 ช่องบนเดือนที่ทองขึ้น 8.68% · ทดสอบจังหวะล้วนรอด Holm 0/36
 *   นี่คือการเปลี่ยน "ปริมาณแจ้งเตือน" ตามที่เจ้าของต้องการ ไม่ใช่การเพิ่มขอบ
 *
 * ⚠ ไม่มีเบรกสำรอง: ประตูคุณภาพบน 15m ผ่าน 410/410 ใบ (ไม่กรองอะไรเลย)
 *   ปริมาณจึงขึ้นตามตารางเป๊ะ ๆ ถ้าเด้งถี่เกินไป ปุ่มที่ต้องหมุนกลับคือบรรทัดนี้
 */
const DEDUPE_HOURS_1D = 20;
const DEDUPE_HOURS_1H = 4;
const DEDUPE_HOURS_15M = 4;
...
async function main() {
  ...
  if (sb) {
    ...
    //
    // ── ทำไม query นี้ถึงไม่มี .eq('status','active') อีกต่อไป (2026-09-03) ────────
    // ตั้งแต่ตัวเก็บผลปั๊ม status ให้ใบที่ปิดบัญชีแล้ว (resolve-signals.mjs) ใบที่โดน SL
    // จะกลายเป็น status='triggered' ถ้ายังกรอง active อยู่ ใบนั้นจะหลุดจากตัวกันซ้ำ
    // แล้ว symbol ที่เพิ่งโดน SL จะออกสัญญาณใหม่ได้เร็วขึ้น = **เปลี่ยนความถี่แจ้งเตือนจริง**
    // ซึ่งไม่ใช่สิ่งที่งานแก้ความถูกต้องของการแสดงผลควรทำเงียบ ๆ
    //
    // กรองด้วย created_at อย่างเดียวให้ผลเท่าเดิมเป๊ะ ไม่ต้องพึ่งคอลัมน์ใหม่สักตัว:
    // ตัวเดียวที่ย้าย status ออกจาก 'active' นอกจากตัวเก็บผลคือการปั๊มหมดอายุข้างล่าง
    // ซึ่งยิงเฉพาะแถวที่เลย expires_at แล้ว และอายุใบสั้นสุด (15m = 24 ชม.) ยังยาวกว่า
    // หน้าต่างกันซ้ำที่กว้างสุด (20 ชม.) เสมอ — จึงไม่มีแถว 'expired' ตัวไหนอยู่ในหน้าต่างนี้ได้
    // ⚠ ถ้าวันไหนย่ออายุใบให้สั้นกว่าหน้าต่างกันซ้ำ ต้องกลับมาคิดเรื่องนี้ใหม่
    const dedupeCols =
      'user_id, symbol, action, timeframe, created_at, id, status' +
    ...
    // ตัวตรวจกลับทิศต้องได้เฉพาะใบที่ยัง active จริง ๆ เหมือนเดิม (การเตือนให้ปิดไม้
    // ที่หมดอายุ/ปิดบัญชีไปแล้ว คือการเตือนผิดใบ) จึงกรอง status ที่นี่แทนใน query
    recentActive = (recent ?? []).filter((r) => r.status === 'active');
    ...
    // ค่าที่ใช้อยู่วันนี้: 1D 20 ชม. · 1H 4 ชม. · 15m 4 ชม. (ทั้งสามอยู่ที่ค่าคงที่ด้านบนไฟล์)
