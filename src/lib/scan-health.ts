...
/**
 * เกณฑ์มาจากพฤติกรรมที่วัดจริง ไม่ใช่ตัวเลขที่ตั้งเอาเอง
 *
 * ตั้ง cron ไว้ทุก 30 นาที แต่ GitHub ส่งจริงเฉลี่ยราว 56 นาที และเคยวัดช่องว่าง
 * ระหว่างรอบได้ตั้งแต่ 15 ถึง 95 นาทีในการใช้งานปกติ (ดูบันทึกใน scan-universe.yml)
 * 2 ชั่วโมงจึงยังอยู่ในวิสัยของ "ช้าตามปกติของ GitHub"
...
export function scanHealth(lastUpdatedIso: string | null | undefined, nowMs = Date.now()): ScanHealth {
  ...
  if (ageMinutes >= SLOW_AFTER_MIN) {
    return {
      ...
      detail: `รอบล่าสุดเมื่อ ${human} · ตั้งไว้ทุก 30 นาทีแต่ GitHub ส่งจริงเฉลี่ยราว 56 นาที และทิ้งรอบไปบ้างเป็นปกติ`,
    };
  }
  return {
    level: 'ok',
