 ...
 *   - 2026-08-29 เจ้าของสั่ง "เน้นแค่ทองอย่างเดียว ไม่เล่นอย่างอื่นแล้ว"
 *     → **เหลือ XAUUSD ตัวเดียว** · XAGUSD ออกด้วยเพราะมันคือโลหะเงิน ไม่ใช่ทอง
 *     (และข้อมูลก็หนุน — ดูเหตุผลรายตัวใน UNSUPPORTED_SYMBOLS)
 *
 * ── ต้นทุนต่อรอบของจักรวาลตอนนี้ (สิ่งที่เป็นจริง "วันนี้") ────────────────────
 *   1 symbol × 3 timeframe (1D 1y · 1H 3mo · 15m 1mo) = **3 คำขอ/รอบ**
 *   วัดจริง 2026-08-31 ด้วย `node scripts/scan-universe.mjs --dry-run --timeframes=1D,1H,15m`
 *   บนเครื่องในไทย 3 รอบ: ทั้งรอบ 1.26–1.99 วิ · ส่วนดึงข้อมูล 0.22–0.23 วิ · ดึงสำเร็จ 3/3
 *   (เวลาส่วนใหญ่คือการโหลด/ลอกชนิด .ts และต่อ Supabase ไม่ใช่การดึงข้อมูล)
 *
 * ⚠ ตัวเลขเวลาทุกบรรทัดข้างล่างนี้วัดบน "จักรวาลที่ไม่มีแล้ว" (57 ตัว = 114 คำขอ)
 *   เก็บไว้เป็นบันทึกว่าเคยคิดด้วยตัวเลขอะไร ไม่ใช่สถานะปัจจุบัน — อย่าเอาไปอ้างว่ารอบนี้ช้าเท่านั้น
 ...
 *
 * ทำไมหยุดที่ 57 ไม่ใช่ 100+ (บันทึกการตัดสินใจเมื่อ 2026-08-17 — ไม่ใช่สถานะปัจจุบัน):
...
/**
 * ตัวเดียวในรายชื่อนี้ผ่านการยิง Yahoo จริงมาแล้วทั้ง 1D (range 1y) · 1H (range 3mo)
 * และ 15m (range 1mo) และได้แท่งเกิน 50 แท่งทุกกรอบ (50 คือขั้นต่ำที่ generateSignal ต้องการ)
 *
 * ⚠ "จักรวาลตัวเดียว" ไม่ใช่อุบัติเหตุ — เจ้าของสั่งเมื่อ 2026-08-29 ว่าเทรดทองอย่างเดียว
 *   ก่อนจะเพิ่มอะไรกลับเข้ามา ให้อ่าน UNSUPPORTED_SYMBOLS ข้างล่างก่อน ทุกตัวที่ถูกถอด
 *   มีเหตุผลกำกับ และ 12 ตัวล่าสุดถูกถอดด้วย "คำสั่งเจ้าของ" ไม่ใช่เพราะดึงข้อมูลไม่ได้
 */
export const SYMBOL_UNIVERSE: readonly UniverseSymbol[] = [
  // ── ทองคำ (1) — 1D 251 แท่ง / 1H 1,448 แท่ง / 15m 2,052 แท่ง ───────────────
  // เจ้าของเทรดทองอย่างเดียวตั้งแต่ 2026-08-29 · XAUUSD คือตัวเดิมใน watchlist อยู่แล้ว
  //
  // ⚠ ทองเป็น symbol ที่ตัวเลขย้อนหลังดีที่สุดในจักรวาลเดิม (ดู signal-evidence.data.json:
  //   XAUUSD|1D|BUY|strong n=163 meanR +0.334 · XAUUSD|1H|BUY|strong n=562 meanR +0.089)
  //   แต่ **ยังไม่เคยพิสูจน์ว่ามี edge นอกตัวอย่าง** — งานวิจัยของรีโปนี้สรุปไว้แล้วว่า
  //   ไม่มีเทคนิคไหนรอด Holm การเหลือทองตัวเดียวจึงคือ "เลิกจ่ายค่าธรรมเนียมให้ช่องอื่น"
  //   ไม่ใช่ "เจอของชัวร์แล้ว"
  { symbol: 'XAUUSD', name: 'ทองคำ', market: 'GOLD' },

  // ⛔ ถอดออกจากจักรวาลแล้วห้ากลุ่ม — เหตุผลคนละแบบ อย่ารวมกัน
  //
  //    0) โลหะเงิน (1) + ค่าเงิน (11) ถอดเมื่อ 2026-08-29 ตามคำสั่งเจ้าของ
  //       "เน้นแค่ทองอย่างเดียวครับ ไม่เล่นอย่างอื่นแล้ว" — รายตัวและตัวเลขที่หนุน
  //       อยู่ใน UNSUPPORTED_SYMBOLS ข้างล่าง (นี่คือรายการที่เอากลับได้ง่ายที่สุด
  //       เพราะทั้ง 12 ตัวยังดึง Yahoo ได้ปกติ ไม่ได้ถูกถอดเพราะข้อมูลเสีย)
  //
  ...
  //      FOREX      −0.0534 [−0.0728, −0.0349]  n=3,889  ← ออกไปแล้ว 2026-08-29
  //      GOLD       −0.0699 [−0.1279, −0.0108]  n=1,345  ← ตลาดเดียวที่ยังอยู่ (แต่นี่คือ
  //                                                        ทอง+เงินรวมกัน ไม่ใช่ของ XAUUSD ล้วน)
  ...
  //    ตอนนั้นที่เหลืออยู่ (ทอง + ค่าเงิน) ก็ยังติดลบทั้งคู่และช่วงความเชื่อมั่นไม่แตะศูนย์
  ...
  //    ข้อนี้ยังจริงกับการเหลือทองตัวเดียวทุกตัวอักษร
  //
  //    ── อยากได้ตัวไหนกลับ ทำแบบนี้ ──────────────────────────────────────────
  //      1. คัดลอกบรรทัด { symbol, name, market } ของตัวนั้นจากประวัติ git:
  //           git show <commit ก่อนคอมมิตนี้>:src/lib/universe.ts
  //         (คอมมิตที่หดจักรวาลเหลือทองคือคอมมิตที่แก้ไฟล์นี้พร้อมกับ scripts/fetch-news.mjs)
  //      2. ลบตัวนั้นออกจาก UNSUPPORTED_SYMBOLS ข้างล่าง ไม่งั้นบันทึกจะขัดกับความจริง
  //      3. แก้จำนวนคำขอ/รอบ ในคอมเมนต์หัวไฟล์ · รายชื่อสำเนาใน scripts/fetch-news.mjs
  //         · และตัวกรอง UNIVERSE_ALLOWED ใน supabase/functions/scan-signals/index.ts
  //           (ชื่อนี้คือชื่อจริงในไฟล์นั้น — npm run test:universe อ่านชื่อนี้ตรง ๆ ด้วย)
  //      4. รัน `node scripts/scan-universe.mjs --dry-run --timeframes=1D,1H,15m` ดูว่า
  //         ดึงข้อมูลได้ครบจริงก่อนคอมมิต · แล้วรัน `npm run test:universe`
];

/**
 * จำนวน symbol ในจักรวาล = 1 (XAUUSD)
 *
 * ⚠ ไม่มีใครในรีโปอ่านค่านี้เลย (grep 2026-08-31: มีแต่บรรทัดที่ประกาศมันเอง)
 *   คอมเมนต์เดิมเขียนว่า "ใช้คำนวณงบเวลาในตัวสแกน (× 2 timeframe = จำนวนคำขอ)"
 *   ซึ่งเท็จสองชั้น: ตัวสแกนใช้ค่าคงที่ CONCURRENCY/FETCH_BUDGET_MS ล้วน ๆ
 *   และเดินอยู่ 3 timeframe ไม่ใช่ 2 — เก็บ export ไว้เพราะเป็น API สาธารณะของไฟล์นี้
 *   ที่ของนอกรีโปอาจอ้างถึง แต่ห้ามเขียนคอมเมนต์ให้มันดูมีหน้าที่ที่ไม่มีจริงอีก
 */
export const UNIVERSE_SIZE = SYMBOL_UNIVERSE.length;

/**
 * ตัวที่ "ไม่อยู่ในจักรวาลแล้ว" — เก็บไว้เป็นบันทึก ไม่ให้ใครเผลอใส่กลับเข้ามาโดยไม่รู้เหตุผล
 * ตัวสแกนไม่ต้องอ่าน export ตัวนี้ มีไว้ให้คนอ่าน
 *
 * ⚠ ในนี้มีสองชนิดปนกัน อย่าสับสน:
 *   · "ดึงข้อมูลไม่ได้จริง" (CNYTHB) — ใส่กลับไปก็ไม่ทำงาน
 *   · "เจ้าของสั่งไม่เล่นแล้ว" (12 ตัวที่เหลือ) — ใส่กลับได้ทันทีถ้าเจ้าของเปลี่ยนใจ
 *     ทุกตัวยังดึง Yahoo ได้ปกติ ณ วันที่ถอด
 */
export const UNSUPPORTED_SYMBOLS: ReadonlyArray<{ symbol: string; market: MarketType; reason: string }> = [
  ...
  // ── ถอดเมื่อ 2026-08-29: เจ้าของสั่ง "เน้นแค่ทองอย่างเดียวครับ ไม่เล่นอย่างอื่นแล้ว" ──
  {
    symbol: 'XAGUSD',
    market: 'GOLD',
    reason:
      'เจ้าของสั่งเมื่อ 2026-08-29 ว่าเทรดทองอย่างเดียว — โลหะเงินอยู่ตลาด GOLD ก็จริง แต่มันคือ "เงิน" ไม่ใช่ "ทอง" ' +
      'และข้อมูลก็หนุนคำสั่งนี้ ไม่ได้ขัด: ต้นทุน 15 bps (XAUUSD 3 bps) แปลว่าต้องตั้ง SL กว้างอย่างน้อย 3.00% ' +
      'ถึงจะกดต้นทุนลงมาที่เพดาน MAX_COST_R = 0.05 R ขณะที่ทองต้องการแค่ 0.60% (ดู minStopPctFor ใน src/lib/costs.ts) ' +
      'และความถี่ในอดีตบน 1H ติดลบทั้งสองทิศ (XAGUSD|1H|BUY meanR −0.0002 n=375 · SELL −0.0165 n=268) ' +
      'ขณะที่ทองเป็นบวกทั้งคู่ (+0.089 / +0.048) — ตัวเลขจาก src/lib/signal-evidence.data.json',
  },
  ...(
    [
      ['EURUSD', 'ยูโร/ดอลลาร์'], ['GBPUSD', 'ปอนด์/ดอลลาร์'], ['USDJPY', 'ดอลลาร์/เยน'],
      ['AUDUSD', 'ดอลลาร์ออสเตรเลีย/ดอลลาร์'], ['USDCHF', 'ดอลลาร์/ฟรังก์สวิส'],
      ['USDCAD', 'ดอลลาร์/ดอลลาร์แคนาดา'], ['NZDUSD', 'ดอลลาร์นิวซีแลนด์/ดอลลาร์'],
      ['EURJPY', 'ยูโร/เยน'], ['GBPJPY', 'ปอนด์/เยน'], ['EURGBP', 'ยูโร/ปอนด์'],
      ['AUDJPY', 'ดอลลาร์ออสเตรเลีย/เยน'],
    ] as const
  ).map(([symbol, name]) => ({
    symbol,
    market: 'FOREX' as MarketType,
    // เขียนเป็น .map เพราะเหตุผลของทั้ง 11 ตัวเป็นอันเดียวกันคำต่อคำ — ถ้าเขียนแยก 11 ก้อน
    // วันที่มีคนแก้เหตุผล จะแก้ไม่ครบแล้วบันทึกจะขัดกันเอง (เคยเกิดกับตาราง COST_BPS มาแล้ว)
    reason:
      `${name} — เจ้าของสั่งเมื่อ 2026-08-29 ว่าเทรดทองอย่างเดียว ไม่ใช่เพราะดึงข้อมูลไม่ได้ ` +
      '(ยังดึง Yahoo ได้ปกติ ณ วันที่ถอด) และไม่ใช่เพราะเพิ่งวัดได้ว่าแย่ — ตัวเลขที่มีคือ FOREX ทั้งตลาด ' +
      'บน validation ได้ −0.0534 R/ไม้ [−0.0728, −0.0349] n=3,889 ซึ่งติดลบมาตั้งแต่ก่อนมีคำสั่งนี้แล้ว',
  })),
];
...
/** อยู่ในจักรวาลที่สแกนจริงไหม — เทียบทั้ง symbol และ market (ตัวพิมพ์ไม่สำคัญ) */
export function isInUniverse(symbol: string, market: string): boolean {
  const key = symbolKey(symbol, market);
  return SYMBOL_UNIVERSE.some((u) => symbolKey(u.symbol, u.market) === key);
}

/**
 * รวมจักรวาลเข้ากับ watchlist ที่ผู้ใช้เพิ่มเอง
 *
 * ── 2026-08-29: watchlist ที่อยู่นอกจักรวาล "ไม่ถูกสแกนอีกต่อไป" ────────────────
 * เดิมฟังก์ชันนี้เอาทุกแถวของ watchlist มารวมกับจักรวาลเสมอ ด้วยเหตุผลว่า
 * "ของที่ผู้ใช้เคยเพิ่มไว้ต้องยังถูกสแกนอยู่ ไม่งั้นเท่ากับลบของเดิมทิ้งเงียบ ๆ"
 * เหตุผลนั้นถูกต้องตอนจักรวาลกับความตั้งใจของเจ้าของยังไปทางเดียวกัน แต่คำสั่งเมื่อ
 * 2026-08-29 ("เน้นแค่ทองอย่างเดียว ไม่เล่นอย่างอื่นแล้ว") ทำให้มันกลายเป็นช่องโหว่:
 * แถว EURUSD/USDTHB ที่ค้างอยู่ในตาราง watchlist จะทำให้ระบบยังออกสัญญาณคู่เงินต่อไป
 * ทุกรอบ ทั้งที่เจ้าของสั่งให้เลิกแล้ว — คือฝ่าฝืนคำสั่งผ่านเส้นทางที่ไม่มีใครมอง
 *
 * จึงกรองด้วยกติกาข้อเดียว: **สแกนเฉพาะสิ่งที่อยู่ใน SYMBOL_UNIVERSE**
 *   · แถว watchlist ที่ตรงกับจักรวาล → ยังสแกนเหมือนเดิม (ทองของใครก็ได้สแกน)
 *   · แถวที่อยู่นอกจักรวาล → ไม่สแกน แต่ **ไม่ลบข้อมูลของผู้ใช้** แถวยังอยู่ในตาราง
 *     หน้า /markets ยังแสดง และติดป้ายบอกตรง ๆ ว่า "ไม่ได้สแกนแล้ว" (ดู src/app/markets/page.tsx)
 * ตัวที่ถูกกรองทิ้งดูได้จาก watchlistOutsideUniverse() ข้างล่าง — ตัวสแกนพิมพ์จำนวนทุกรอบ
 * เพื่อไม่ให้ "หายไปเงียบ ๆ" ซึ่งเป็นสิ่งที่คอมเมนต์เดิมกลัวไว้ถูกแล้ว
 */
export function buildScanTargets(
  ...
  for (const row of watchlistRows) {
    ...
    // นอกจักรวาล = ไม่สแกน (คำสั่งเจ้าของ 2026-08-29) — ดูคอมเมนต์หัวฟังก์ชัน
  }
...
/**
 * แถว watchlist ที่ "จะไม่ถูกสแกน" เพราะอยู่นอกจักรวาล — ไม่ซ้ำกัน เรียงตามที่พบ
 * มีไว้ให้ผู้เรียกรายงานเป็นตัวเลขได้ ไม่ใช่ให้ทิ้งเงียบ ๆ
 */
export function watchlistOutsideUniverse(
  watchlistRows: ReadonlyArray<{ symbol: string; name?: string | null; market: MarketType }> = []
): Array<{ symbol: string; market: MarketType }> {
  const seen = new Set<string>();
  const out: Array<{ symbol: string; market: MarketType }> = [];
  for (const row of watchlistRows) {
    if (!row?.symbol || !row?.market) continue;
    const symbol = row.symbol.trim().toUpperCase();
    const key = symbolKey(symbol, row.market);
    if (seen.has(key) || isInUniverse(symbol, row.market)) continue;
    seen.add(key);
    out.push({ symbol, market: row.market });
  }
  return out;
}
...
export const SIGNAL_GATE: SignalGateConfig = {
   ...
   *
   * ── ทำไมเปลี่ยนจาก 5 เป็น 3 เมื่อ 2026-08-29 (และทำไมมันไม่ได้ตัดอะไรทิ้งเลย) ──
   * จักรวาลเหลือ XAUUSD ตัวเดียว และ generateSignal ให้สัญญาณได้อย่างมาก 1 อัน
   * ต่อ symbol ต่อ timeframe → เพดานที่เป็นไปได้จริงคือ 1 × 3 timeframe = **3 อัน/รอบ**
   * เลข 5 จึงเป็นเพดานที่เอื้อมไม่ถึงตลอดกาล (over_run_limit จะเป็น 0 ตลอด) ซึ่งอันตราย
   * กว่าที่คิด: มันทำให้คนอ่านเชื่อว่ายังมีตัวคุมจำนวนอยู่ ทั้งที่ตัวคุมตัวจริงคือ
   * "จักรวาลมีตัวเดียว" ต่างหาก · ตั้งเป็น 3 = พูดความจริงว่าเพดานเท่ากับของที่มีอยู่จริง
   * และ **ไม่มีสัญญาณใบไหนถูกตัดเพิ่มแม้แต่ใบเดียว** (3 คือจำนวนสูงสุดที่ผลิตได้อยู่แล้ว)
   *
   * ⚠ วันที่เพิ่ม symbol กลับเข้าจักรวาล ต้องกลับมาคิดเลขนี้ใหม่ทันที ไม่งั้นจะกลายเป็น
   *   เพดานที่ "กัดจริง" โดยไม่มีใครตั้งใจ — เพดานเดิม 5 มาจากการวัดรอบจริง 252 รอบ
   *   (เกิน 5 อันมีแค่ 2 รอบ = 0.8% · เกิน 3 อันมี 6 รอบ = 2.4%) บนจักรวาล 13 ตัว
   *   ซึ่งเป็นจักรวาลที่ไม่มีแล้ว
   *
  ...
  perTimeframe: {
    '15m': {
      ...
      minRiskReward: 1.2,
      /**
       * ⚠ ค่านี้ "ไม่เคยถูกใช้" และไม่ใช่ของใหม่ — selectSignals() อ่านเพดานต่อรอบจาก
       *   gate ชั้นบนที่ถูกส่งเข้ามาเท่านั้น (ดูบรรทัด ranked.slice(0, gate.maxSignalsPerRun))
       *   ส่วน gateForTimeframe() ถูกเรียกใน evaluateSignal() ซึ่งตัดสิน "ทีละสัญญาณ"
       *   จึงไม่มีเส้นทางไหนที่ค่า 8 นี้จะมีผล · เพดานจริงของทุก timeframe คือ 3 ข้างบน
       *   เก็บไว้เพราะเป็นข้อจำกัดเดิมที่มีมาก่อนการหดจักรวาล การลบทิ้งเป็นการแก้คนละเรื่อง
       *   (และถ้าจะแก้ให้ทำงานจริง ต้องแก้ selectSignals ให้แยกโควตาตาม timeframe ก่อน)
       */
      maxSignalsPerRun: 8,
    },
  },
 ...
 *   ได้ไม้ที่ดีกว่าเรียงแบบอื่น ข่าวดีคือตั้งแต่ 2026-08-29 มันไม่มีผลเลย ไม่ใช่แค่ "แทบ":
 *   จักรวาลเหลือ 1 symbol × 3 timeframe = อย่างมาก 3 ใบ/รอบ ซึ่งเท่ากับเพดานพอดี
 *   ทุกใบที่ผ่านเกณฑ์จึงได้ไปต่อเสมอ ลำดับมีผลแค่กับ "ลำดับที่เห็นในแจ้งเตือน" เท่านั้น
 *   (ตอนจักรวาลยังมี 13 ตัว เพดาน 5 ไปแตะแค่ 0.8% ของรอบทั้งหมดอยู่แล้ว)
 */
