...
export interface Signal {
  ...
  // เขียนโดย scripts/scan-universe.mjs บน "ใบเก่า" เมื่อเครื่องยนต์ออกสัญญาณ
  // ทิศตรงข้ามบน symbol+timeframe เดียวกันขณะใบนี้ยังเปิดอยู่
  // เป็น optional เพราะโหมดถอย: เจ้าของยังไม่ได้รัน 009 = คอลัมน์ไม่มี = field ไม่มา
  // UI ต้องทำงานได้ปกติ (แค่ไม่ขึ้นป้าย) ห้ามพัง
  // ⚠ flipped ≠ closed — ป้ายนี้ไม่ปิดบัญชี ตัวเก็บผลยัง resolve ใบนี้ตามปกติ
  flipped_at?: string | null;
  /** id ของใบทิศตรงข้ามที่มากลับทิศใบนี้ (soft reference — ไม่มี FK) */
  flipped_by?: string | null;

  /**
   * field ในหน่วยความจำเท่านั้น — ตาราง signals **ไม่มีคอลัมน์นี้**
   * ตัวสแกนแนบให้ "ใบใหม่" ที่กลับทิศใบเก่า (แบบเดียวกับ `evidence`) เพื่อให้
   * push-digest ประกอบบรรทัดเตือนได้ แล้วถอดออกก่อน insert เสมอ
   * ใบที่อ่านกลับจาก DB จะไม่มีค่านี้ติดมาเอง — push-server ประกอบคืนจากป้าย
   * flipped_by ให้ใบเก็บตก (รอบที่ถูกกันความถี่/ส่งล้ม) ก่อนประกอบใบแจ้งเตือน
   * ยังไม่ได้รัน 009 = ไม่มีป้ายให้ประกอบคืน ใบเก็บตกจะไม่มีบรรทัดเตือน
   */
  flip?: SignalFlipNote | null;
}

/** ข้อมูลแนบของ "ใบใหม่ที่กลับทิศใบเก่า" — อยู่ในหน่วยความจำของรอบสแกนเท่านั้น */
export interface SignalFlipNote {
  /** action ของใบเก่าล่าสุดที่โดนกลับทิศ (ของจริงจากแถวนั้น ไม่ใช่คำกลาง ๆ) */
  prev_action: SignalAction;
  /** created_at ของใบเก่าล่าสุด — เอาไปพิมพ์ "ที่ส่งเมื่อ HH:mm" ในคำเตือน */
  prev_created_at: string | null;
  /** id ของใบเก่าทุกใบที่โดนกลับทิศ — ตัวสแกนใช้ปั๊ม flipped_at/flipped_by หลัง insert สำเร็จ */
  target_ids?: string[];
}
