...
async function fetchChart(
  ...
  for (const host of CHART_HOSTS) {
    try {
      ...
      // generateSignal กรองซ้ำอีกชั้นก็จริง แต่ quote (price/high/low ล่าสุด) ประกอบจาก candles
      // ตรงนี้ จึงต้องกรองก่อนใช้เหมือนต้นฉบับ
...
// ═══════════════════════════════════════════════════════════════════════════
// "แจ้งกลับทิศ" (migration 009) อยู่ฝั่ง scripts/scan-universe.mjs เท่านั้น — ตั้งใจ
//
// ฟีเจอร์: เมื่อออกสัญญาณทิศตรงข้ามกับใบเดิมที่ยังเปิดอยู่ (symbol+timeframe เดียวกัน)
// ตัวสแกนหลักจะปั๊ม flipped_at/flipped_by บนใบเก่า + ต่อบรรทัดเตือนในใบแจ้งเตือน
//
// ทำไมไฟล์นี้ไม่ทำด้วย:
//  · ตรรกะ flip อยู่ชั้น "ท่อ" (ข้อมูลจาก query กันซ้ำ + การประกอบใบ digest ของ
//    push-digest.ts) ไม่ได้อยู่ใน indicators/signal-engine ที่ไฟล์นี้มีสำเนา —
//    check-scan-parity.mjs จึงไม่บังคับให้ sync และบล็อกสำเนาทั้งสามไม่ต้องแตะ
//  · กติกาของ repo: สำเนาที่ไม่มีเครื่องตรวจ parity คุม = ห้ามมี (ดูหมายเหตุ Web Push
//    ข้างบนที่ยึดหลักเดียวกัน) การลอกตรรกะ flip มาไว้ที่นี่โดยไม่มีด่านตรวจ
//    คือการสร้างสำเนาที่จะเพี้ยนเงียบ ๆ แบบที่เคยเกิดมาแล้ว
//  · ไฟล์นี้เป็นแค่ตัวสำรอง "สแกน + บันทึก" — แถวที่มัน insert จะถูกแจ้งเตือนโดย
//    ตัวส่งกลางของตัวสแกนหลักอยู่แล้ว (push_sent = false)
//
// ผลข้างเคียงที่รู้และยอมรับ: ใบทิศตรงข้ามที่ "ไฟล์นี้" insert จะไม่ปั๊มป้ายให้ใบเก่า
// (ป้ายบนเว็บไม่ขึ้นสำหรับคู่นั้น จนกว่าตัวสแกนหลักจะออกสัญญาณทิศตรงข้ามเอง)
// ถ้าวันหนึ่งจะย้ายฟีเจอร์นี้เข้ามา ต้องทำเป็นสำเนาที่มีเครื่องตรวจ parity ของตัวเองก่อน
// ═══════════════════════════════════════════════════════════════════════════
